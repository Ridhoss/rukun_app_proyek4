...
class _AddKKPageState extends State<AddKKPage> {
  ...
  void initState() {
    ...
    if (widget.editData != null) {
      ...
      _addressCtrl.text = widget.editData!.address;
    }
  }
  ...
  Widget _buildScaffold(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        title: Text(
          widget.editData != null ? 'Edit Kartu Keluarga' : 'Tambah Kartu Keluarga',
        ...
        actions: _vm.kkSaved
          ...
          TextButton(
            ...
            child: const Text('Selesai',
                style: TextStyle(color: ColorsUtils.white, fontWeight: FontWeight.w600)),
          )
        ]
            : null,
      ),
      ...
      bottomNavigationBar: _vm.kkSaved ? _buildBottomBar() : const SizedBox.shrink(),
    );
  }
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          TextButton(
            ...
            child: const Text('Tambah dulu', style: TextStyle(color: ColorsUtils.b500)),
          ),
          ElevatedButton(
  ...
  void _goToAddWarga() {
    Navigator.push<WargaModel>(
      context,
      MaterialPageRoute(builder: (_) => AddWargaPage(keluargaId: _vm.savedKKId)),
    ).then((warga) { if (warga != null) _vm.addAnggota(warga); });
  }
  ...
  void _goToEditWarga(int index) {
    ...
    ).then((updated) { if (updated != null) _vm.updateAnggota(index, updated); });
  }
  ...
  void _confirmDeleteWarga(int index) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
        title: const Text('Hapus Anggota?', style: TextStyle(fontWeight: FontWeight.w700)),
        content: Text('Yakin hapus "${_vm.anggotaList[index].nama}"?'),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(ctx),
            child: const Text('Batal', style: TextStyle(color: ColorsUtils.gray)),
          ),
          ElevatedButton(
            style: ElevatedButton.styleFrom(
              backgroundColor: Colors.red, foregroundColor: ColorsUtils.white,
              shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8)),
            ),
            onPressed: () {
              Navigator.pop(ctx);
  ...
  Widget _buildStepIndicator() {
    return Container(
      ...
      child: Row(children: [
        _buildStep(number: '1', label: 'Data KK', active: true, done: _vm.kkSaved),
        _buildStepLine(active: _vm.kkSaved),
        _buildStep(number: '2', label: 'Anggota', active: _vm.kkSaved, done: false),
      ]),
    );
  }

  Widget _buildStep({required String number, required String label, required bool active, required bool done}) {
    return Column(children: [
      Container(
        width: 32, height: 32,
        decoration: BoxDecoration(shape: BoxShape.circle,
            color: done ? Colors.green : active ? ColorsUtils.white : ColorsUtils.b400),
        child: Center(child: done
            ? const Icon(Icons.check, size: 16, color: Colors.white)
            : Text(number, style: TextStyle(fontWeight: FontWeight.w700, fontSize: 14,
            color: active ? ColorsUtils.b500 : Colors.white54))),
      ),
      const SizedBox(height: 4),
      Text(label, style: TextStyle(fontSize: 11, color: active ? ColorsUtils.white : Colors.white54,
  ...
  Widget _buildStepLine({required bool active}) {
    return Expanded(child: Container(
      height: 2, margin: const EdgeInsets.only(bottom: 20, left: 8, right: 8),
      ...
      child: Form(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          _vm.isLoadingRT
              ...
              child: CircularProgressIndicator(strokeWidth: 2, color: ColorsUtils.b500)))
              : DropdownButtonFormField<int>(
            value: _vm.selectedRTId,
            decoration: _inputDecoration(),
            isExpanded: true,
            hint: Text('Pilih RT', style: TextStyle(color: Colors.grey[400], fontSize: 14)),
            items: _vm.rtList.map((rt) => DropdownMenuItem<int>(
                value: rt['id'] as int,
                child: Text(rt['name'] as String, style: const TextStyle(fontSize: 14)))).toList(),
          ...
          TextFormField(
            ...
            maxLines: 3,
            enabled: !_vm.kkSaved,
            decoration: _inputDecoration(hint: 'Jl. nama jalan, No. rumah, RT/RW...'),
            ...
            SizedBox(width: double.infinity, height: 46,
              child: ElevatedButton.icon(
                icon: _vm.isLoadingKK
                    ...
                    child: CircularProgressIndicator(strokeWidth: 2, color: ColorsUtils.white))
                    : const Icon(Icons.save_outlined, size: 18),
                label: const Text('Simpan Data KK',
          ...
          if (_vm.kkSaved)
            TextButton.icon(
              icon: const Icon(Icons.edit_outlined, size: 16, color: ColorsUtils.b500),
              label: const Text('Ubah Data KK',
                  style: TextStyle(color: ColorsUtils.b500, fontWeight: FontWeight.w600)),
  ...
  Widget _buildAnggotaSection(BuildContext context) {
    return _buildCard(
      header: _buildSectionHeader('Anggota Keluarga', Icons.people_outline,
          trailing: _buildBadge('${_vm.anggotaList.length} orang', ColorsUtils.b500)),
      child: Column(children: [
        if (!_vm.kkSaved)
          _buildInfoBox('Simpan data KK terlebih dahulu sebelum menambah anggota keluarga.')
        else ...[
          if (_vm.anggotaList.isEmpty) _buildEmptyAnggota(),
          ..._vm.anggotaList.asMap().entries.map((e) =>
              Padding(padding: const EdgeInsets.only(bottom: 10),
                  child: _buildAnggotaCard(e.value, e.key))),
          const SizedBox(height: 12),
          SizedBox(width: double.infinity, height: 46,
            child: OutlinedButton.icon(
              icon: const Icon(Icons.person_add_outlined, size: 18, color: ColorsUtils.b500),
    ...
    return const Padding(
      ...
      child: Column(children: [
        ...
        Text('Belum ada anggota keluarga',
            style: TextStyle(color: ColorsUtils.gray, fontWeight: FontWeight.w500)),
        SizedBox(height: 4),
  ...
  Widget _buildAnggotaCard(WargaModel warga, int index) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: isKK ? ColorsUtils.b75 : const Color(0xFFE5E7EB)),
      ),
      child: Row(children: [
        Container(
          width: 42, height: 42,
          decoration: BoxDecoration(shape: BoxShape.circle,
              color: isKK ? ColorsUtils.b500 : ColorsUtils.gray.withOpacity(0.15)),
          child: Center(child: Text(
            warga.nama.isNotEmpty ? warga.nama[0].toUpperCase() : '?',
            style: TextStyle(color: isKK ? ColorsUtils.white : ColorsUtils.gray,
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            Text(warga.nama, style: const TextStyle(fontWeight: FontWeight.w700,
                fontSize: 14, color: ColorsUtils.black800)),
            if (isKK) ...[
              const SizedBox(width: 6),
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 7, vertical: 2),
                decoration: BoxDecoration(color: ColorsUtils.b500, borderRadius: BorderRadius.circular(4)),
                child: const Text('KK', style: TextStyle(color: ColorsUtils.white, fontSize: 10, fontWeight: FontWeight.w700)),
              ),
            ],
          ]),
          const SizedBox(height: 3),
          Text('${warga.nik} · ${warga.statusHubungan}',
              style: const TextStyle(color: ColorsUtils.gray, fontSize: 12)),
        ])),
        PopupMenuButton<String>(
          icon: const Icon(Icons.more_vert, size: 18, color: ColorsUtils.gray),
          onSelected: (val) {
    ...
    return Container(
      ...
      child: SizedBox(
        ...
        child: ElevatedButton(
          onPressed: _onSelesai,
          style: ElevatedButton.styleFrom(
            backgroundColor: _vm.anggotaList.isNotEmpty ? Colors.green : ColorsUtils.b500,
            foregroundColor: ColorsUtils.white,
            shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10)),
            elevation: 0,
          ),
          child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
            Icon(_vm.anggotaList.isNotEmpty ? Icons.check_circle_outline : Icons.arrow_forward, size: 18),
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        Expanded(child: Text(title, style: const TextStyle(
            fontSize: 14, fontWeight: FontWeight.w700, color: ColorsUtils.b400))),
  ...
  InputDecoration _inputDecoration({String? hint}) {
    return InputDecoration(
      ...
      disabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(8), borderSide: const BorderSide(color: Color(0xFFEEF0F5))),
